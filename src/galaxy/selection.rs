...
fn resolve_proxies(
    ...
    for mut proxy in proxies.iter_mut() {
        proxy.resolved_target = match proxy.target {
            ...
            */
            InterfaceIdentifier::PlayerEmpire => player_empire.empire,
            InterfaceIdentifier::CurrentSystemOrbiter(i) => selection
                .selected_system
            ...
            InterfaceIdentifier::CurrentSelectedFleet(i) => {
                if let Some(selected) = selection.selected {
                    if i == 0 && fleet_query.contains(selected) {
                        selection.selected
                    } else {
                        None
                    }
...
fn update_hovered(
    ...
    {
        if let Some(proxy) = selection_proxy_query
            .get(hovered_entity)
            .ok()
            .and_then(|x| x.resolved_target)
        {
...
fn update_selection(
        ...
        .filter(|pointer| pointer.event.button == PointerButton::Primary)
    {
        ...
        let target = selection_proxy_query
            .get(*target)
            .ok()
            .and_then(|x| x.resolved_target)
            .unwrap_or(*target);
