...
fn setup_widget(mut commands: Commands, mut materials: ResMut<Assets<StarIconMaterial>>) {
    commands
        .spawn((
            SelectionPanel,
            Node {
...
fn update_widget_system(
    ...
    if selection.is_changed() {
        ...
        let Ok(empire_index) = empires_index.get(empire) else {
            return;
        };
        let empire_stars = &empire_index.systems;
        let desc = empire_stars
            .iter()
            .map(|x| star_query.get(*x).unwrap())
            .collect::<Vec<_>>();
        ...
        for (mut text, panel) in header_query.iter_mut() {
            if panel.slot < len {
                ...
                text.0 = format!(
                    "{} ({})",
                    desc[panel.slot as usize].0.name,
                    desc[panel.slot as usize].1.population.format_big_number()
                );
